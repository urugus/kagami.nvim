...
const getScrollTop = (height) => {
    let scrollTop = Math.max(0, (state.scroll?.topline || 1) - 1);
    if (!state.options?.follow_scroll)
        scrollTop = 0;
...
const draw = async () => {
    ...
    if (mode === "sixel") {
        ...
        const blocksEnabled = String(process.env.KAGAMI_SIXEL_BLOCKS || "").toLowerCase() === "1" || mermaidEnabled;
        clearScreen();
        if (blocksEnabled) {
            ...
            for (const block of blocks) {
                ...
                const isMermaidFence = mermaidEnabled && first === "```mermaid" && last === "```" && blockLines.length >= 3;
                if (isMermaidFence) {
                    ...
                    if (sixel) {
                        ...
                        const fallback = await renderSixelFromText(`(mermaid render failed)\n${error || "unknown error"}`, width, 3);
