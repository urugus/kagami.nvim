...
const run = (cmd, args, stdinText) => new Promise((resolve) => {
...
const coerceEnvNumber = (v, fallback) => {
    const n = Number(v);
    if (!Number.isFinite(n))
        return fallback;
    return n;
};
const estimateRows = (code, maxRows) => {
    const lines = code.split("\n").length;
    return Math.max(8, Math.min(maxRows, Math.ceil(lines * 2)));
};
export const renderSixelFromMermaid = async (code, cols, maxRows) => {
    const mode = (process.env.KAGAMI_MODE || "ansi").toLowerCase();
    if (mode !== "sixel")
        return { sixel: null, error: null };
    ...
    if (cached)
        return { sixel: cached, error: null };
